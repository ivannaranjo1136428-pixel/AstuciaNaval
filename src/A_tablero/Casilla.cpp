...
void Casilla::Mostrar(bool propio) const { std::cout << ToString(propio); }
void Casilla::MostrarGuia() const { std::cout << ToStringGuia(); }

std::string Casilla::ToString(bool propio) const {
    if (hundido) return "\033[45m  \033[0m";     // morado
    else if (disparado) {
        if (ocupado) return "\033[41m  \033[0m"; // rojo
        else return "\033[44m  \033[0m";         // azul
    } else {
        if (propio && ocupado) return "\033[42m  \033[0m"; // verde
        else return "\033[47m  \033[0m";                   // blanco
    }
}

std::string Casilla::ToStringGuia() const {
    if (hundido) return "\033[45m  \033[0m";
    else if (disparado) {
        if (ocupado) return "\033[41m  \033[0m";
        else return "\033[44m  \033[0m";
    } else return "\033[47m  \033[0m";
}
